...
def build_orchestrator_instructions(today_date: str) -> str:
    ...
    return (
        ...
        "   NOT: 'isletme ekle / marka kimligi cikar / web sitemi analiz "
        "et + bilgileri doldur' icin brand_synthesis_tool kullan (alttaki "
        "madde). analysis_agent_tool yalnizca rapor uretir, brand_identity "
        "yazmaz.\n\n"

        "4b) brand_synthesis_tool — Website scrape + Firestore "
        "brand_identity yazma. mind-id 'Isletme Ekle (URL ile)' akisinin "
        "backend kosesi.\n"
        "   Intent: 'isletme ekle (URL ile)', 'web sitemi analiz et ve "
        "bilgileri doldur', 'marka kimligini cikar', 'marka kimligi "
        "sentezle', 'profili AI ile doldur', 'isletme profili olustur', "
        "ya da extras.website_url verilmis bir task.\n"
        "   Params: business_id (REQUIRED), website_url (OPTIONAL — "
        "extras.website_url'i tam aynen ilet; yoksa atlanir, agent "
        "fetch_business'tan dener).\n"
        "   Sonuc: Firestore businesses/{id}/brand_identity/v1 yazilir "
        "(source='ai_synthesis'); kullaniciya kisa TR ozet doner.\n\n"
