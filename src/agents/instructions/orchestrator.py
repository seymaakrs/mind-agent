...
def build_orchestrator_instructions(today_date: str) -> str:
    ...
    return (
        ...
        "🔥🔥🔥 PRIORITY RULE #0 - LEAD/CRM ROUTING (READ THIS FIRST, OVERRIDES EVERYTHING ELSE) 🔥🔥🔥\n"
        "If the user's message contains the word 'lead' (in ANY form: lead, leads, leadler, leadleri, leadlerin, sıcak lead, soğuk lead, sicak lead, soguk lead, kaç lead, kac lead, lead listesi, lead sayısı, hangi leadler, leadleri listele, lead göster, lead getir, lead durumu, lead aşaması, etc.), you MUST follow these steps:\n"
        "  STEP 1: SKIP fetch_business. SKIP query_documents. SKIP get_document. SKIP analysis_agent_tool. SKIP marketing_agent_tool.\n"
        "  STEP 2: Call meta_agent_tool DIRECTLY with the user's question and the business_id from [Business ID: xxx].\n"
        "  STEP 3: Return meta_agent_tool's response.\n"
        "Examples:\n"
        "  - User: 'kaç sıcak lead var, listele' → meta_agent_tool(business_id='xxx', task='kaç sıcak lead var, listele - query_leads ile (asama,eq,Sicak) filtresi kullan')\n"
        "  - User: 'soğuk leadleri göster' → meta_agent_tool(...)\n"
        "  - User: 'bugünkü leadler kim' → meta_agent_tool(...)\n"
        "Lead data is in NocoDB CRM, accessed ONLY via meta_agent_tool. query_documents reads Firestore (reports/plans/memory) - it has NO leads. analysis_agent_tool is for SEO/SWOT/research - it has NO leads.\n"
        "VIOLATION = WRONG ANSWER. If you call ANY tool other than meta_agent_tool for a lead query, you have failed the task.\n\n"
        ...
        "- meta_agent_tool: for FACEBOOK/META LEAD ADS processing AND NocoDB CRM lead queries — incoming Lead Form submissions, Meta lead reports, OR querying/listing/counting existing leads (lead, lead form, lead ads, meta lead, facebook lead, instagram lead, sicak lead, soguk lead, lead skor, NocoDB CRM, kac lead, lead listele, lead sorgula, lead say, CRM sorgu)"
        "\n\n"
